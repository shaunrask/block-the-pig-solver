def generate_problem():
    ...
    # Format Output
    output = []
    output.append("{:name \"Block the Pig Trap Test Small\"")
    output.append(" :description \"Verifying trapped condition on small grid\"")
    ...
    # Goal: Prove that all neighbors of C_0_0 have walls (Explicit Conjunction)
    # (and (HasWall C_1_0) ...)
    goal_terms = []
    for n in neighbors_0_0:
        goal_terms.append(f"(HasWall {n})")
    
    goal_str = f"(and {' '.join(goal_terms)})"
    output.append(f" :goal {goal_str}")
...
if __name__ == "__main__":
    with open("shadowprover/test_trap_small.clj", "w", encoding="utf-8") as f:
